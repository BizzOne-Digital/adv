import { ZodError } from "zod";

import {
  escapeRegex,
  jsonError,
  jsonOk,
  logActivity,
  parsePagination,
  requireAdmin,
  zodErrorResponse,
} from "@/lib/admin";
import { CACHE_TAGS, revalidateContent } from "@/lib/revalidate";
import { teamSchema } from "@/lib/validations";
import { TeamMember } from "@/models/TeamMember";

export async function GET(request: Request) {
  const result = await requireAdmin();
  if (result.error) return result.error;
  const { searchParams } = new URL(request.url);
  const { page, limit, skip, q, status } = parsePagination(searchParams);
  const filter: Record<string, unknown> = {};
  if (status) filter.status = status;
  if (q) {
    filter.$or = [
      { name: { $regex: escapeRegex(q), $options: "i" } },
      { slug: { $regex: escapeRegex(q), $options: "i" } },
      { role: { $regex: escapeRegex(q), $options: "i" } },
      { department: { $regex: escapeRegex(q), $options: "i" } },
    ];
  }
  const [items, total] = await Promise.all([
    TeamMember.find(filter).sort({ order: 1, name: 1 }).skip(skip).limit(limit).lean(),
    TeamMember.countDocuments(filter),
  ]);
  return jsonOk({ items, total, page, limit });
}

export async function POST(request: Request) {
  const result = await requireAdmin();
  if (result.error) return result.error;
  try {
    const data = teamSchema.parse(await request.json());
    if (await TeamMember.findOne({ slug: data.slug })) {
      return jsonError("A team member with this slug already exists", 409);
    }
    const doc = await TeamMember.create(data);
    await logActivity({
      session: result.session,
      action: "create",
      entityType: "team",
      entityId: String(doc._id),
      summary: `Created team member “${doc.name}”`,
      request,
    });
    if (doc.status === "published") {
      revalidateContent({ tags: [CACHE_TAGS.team], paths: ["/team"] });
    }
    return jsonOk(doc, "Team member created", 201);
  } catch (error) {
    if (error instanceof ZodError) return zodErrorResponse(error);
    return jsonError("Failed to create team member", 500);
  }
}
